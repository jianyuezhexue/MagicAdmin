<template>
  <span class="headerAvatar">
    <template v-if="picType === 'avatar'">
      <el-avatar v-if="userStore.userInfo.headImg" :size="30" :src="avatar" />
      <el-avatar v-else :size="30" :src="noAvatar" />
    </template>
    <template v-if="picType === 'img'">
      <img v-if="userStore.userInfo.headImg" :src="avatar" class="avatar">
...
const avatar = computed(() => {
  if (props.picSrc === '') {
    if (userStore.userInfo.headImg !== '' && userStore.userInfo.headImg.slice(0, 4) === 'http') {
      return userStore.userInfo.headImg
    }
    return path.value + userStore.userInfo.headImg
  } else {
